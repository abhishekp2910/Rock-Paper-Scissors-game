...
function updateScores(resultMsg) {
    ...
    checkForWinner();
    matchCounter(matchesWon,matchesLost);
}
...
function checkForWinner() {
    ...
    if (pScore === WINNING_SCORE) {
        ...
        let reset=setTimeout(resetScores,3000);
        console.log(reset);
        matchesWon++;
       ...
       document.querySelector("#userMove").innerText=`YOU LOSE`;
       reset=setTimeout(resetScores,3000);
       console.log(reset);
       matchesLost++;
...
function matchCounter(matchesWon,matchesLost) {

    document.querySelector('#mw').innerText = `Matches Won: ${this.matchesWon}`;
    document.querySelector('#ml').innerText = `Matches Lost: ${this.matchesLost}`;
